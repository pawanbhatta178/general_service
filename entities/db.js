...
const {
  arrayToOrderList,
  arrayToOrderListWithQuote,
} = require("./helper/arrayToOrderList");
...
const db = (model) => {
  return {
    ...
    save: async () => {
      ...
      let query = `INSERT INTO ${modelName} ${arrayToOrderList(
        keysArray
      )} VALUES ${arrayToOrderListWithQuote(valuesArray)}  RETURNING *;`;
      try {
    ...
    findOneWith: async (constraints) => {
      ...
      let query = `SELECT * FROM ${tableName} where ${constraintsToFilterFormula(
        constraints
      )} LIMIT 1`;
      try {
        const { rows } = await pool.query(query);
    ...
    deleteAllWith: async (constraints) => {
      const tableName = model.constructor.name;
      let query = `DELETE FROM ${tableName} where ${constraintsToFilterFormula(
        constraints
      )} RETURNING *`;
      try {
        const { rows } = await pool.query(query);
    ...
    updateOneWith: async (filterConstraints, dataToUpdate) => {
      const tableName = model.constructor.name;
      let query = `UPDATE ${tableName} SET ${getSetCondition(
        dataToUpdate
      )} where ${constraintsToFilterFormula(filterConstraints)} RETURNING *`;
      try {
        const { rows } = await pool.query(query);
        return rows;
      } catch (err) {
